...
class ProgramCourseAssessment(BaseModel):
    ...
    exam_category = relationship('ExamCategory', lazy='noload', back_populates="program_course_assessments")

    program_course_id: int = Column(Integer, ForeignKey("program_courses.id"),  nullable=False, unique=False)
    program_course = relationship('ProgramCourse', lazy='noload', back_populates="program_course_assessments")
